...
export const chatMessageReceived = (
    ...
    socket.on("message_received", (data: IMessageDocument) => {
        ...
        if (data.conversationId === conversationId) {
            ...
            const unique = chatMessages.filter(
                (
                    item: IMessageDocument,
                    index: number,
                    list: IMessageDocument[]
                ) => {
                    const itemIndex = list.findIndex(
                        (listItem: IMessageDocument) =>
                            listItem._id === item._id
                    );
                    return itemIndex === index;
                }
            );
            setChatMessagesData(unique);
        }
    });
};
...
export const chatListMessageReceived = (
    ...
    socket.on("message_received", (data: IMessageDocument) => {
        ...
        if (
            ...
            const messageIndex = findIndex(chatList, [
                "conversationId",
                data.conversationId
            ]);
            if (messageIndex > -1) {
            ...
            if (lowerCase(data.receiverUsername!) === lowerCase(username)) {
                ...
                dispatch(
                    updateNotification({ hasUnreadMessage: list.length > 0 })
                );
            }
