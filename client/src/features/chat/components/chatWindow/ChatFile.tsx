...
import { checkUrlExtension } from "src/shared/utils/image-util.service";
import {
  bytesToSize,
  downloadFile,
  getFileBlob,
    ...
    <div className="flex w-64 min-w-[100%] flex-col">
      <div className="z-1 mt-2 flex flex-col rounded">
        {checkUrlExtension(`${message.fileType}`) === "image" && (
          <img className="h-36 w-64 object-cover" src={message.file} alt="" />
        )}
      ...
      <div className="flex w-auto justify-between">
        <div
          ...
          onClick={() =>
            downloadChatFile(`${message.file}`, `${message.fileName}`)
          }
        >
        ...
        <span className="truncate text-xs md:text-sm">
          ({bytesToSize(parseInt(`${message.fileSize}`))})
        </span>
      </div>
    </div>
  );
