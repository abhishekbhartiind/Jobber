...
import { IChatMessageProps } from "../../service/interfaces/chat.interface";
...
const ChatOffer: FC<IChatMessageProps> = ({
  ...
  const updateBuyerOffer = async (
    ...
    try {
      ...
      if (type === "accepted") {
        navigate(
          `/gig/checkout/${message.gigId}?${createSearchParams({ offer: JSON.stringify(offerParams) })}`,
          { state: gig }
        );
      }
    } catch (error) {
  ...
  return (
    <div className="z-1 border-grey mt-2 flex py-2 max-w-xl flex-col overflow-hidden rounded border">
      <div className="w-full">
        ...
        <div className="border-grey flex flex-row gap-x-2 border-b px-4 py-3 text-sm font-bold md:text-base">
          <FaRegClock className="self-center" /> {messageOffer.deliveryInDays}{" "}
          Day
          {parseInt(`${messageOffer.deliveryInDays}`) > 1 ? "s" : ""} Delivery
        </div>
        <div className="relative top-[5%] mr-3 flex flex-row justify-end gap-4">
          <Button
            className={`rounded px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base mt-3 ${
            ...
            label={
              seller?._id === message.sellerId ? "Cancel Offer" : "Reject Offer"
            }
            onClick={() =>
