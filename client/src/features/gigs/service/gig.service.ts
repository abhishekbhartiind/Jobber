...
export const gigsApi = api.injectEndpoints({
    endpoints: (build) => ({
        ...
        getSellerInactiveGigs: build.query<IResponse, string>({
            query: (sellerId: string) => `gig/seller/inactive/${sellerId}`,
            providesTags: ["Gigs"]
        }),
        ...
        updateActiveGig: build.mutation<
            IResponse,
            { gigId: string; active: boolean }
        >({
            query({ gigId, active }) {
                return {
                    url: `gig/active-status/${gigId}`,
                    method: "PUT",
                    body: { active }
                };
            },
