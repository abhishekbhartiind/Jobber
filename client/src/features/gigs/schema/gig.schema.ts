...
const gigInfoSchema: ObjectSchema<ICreateGig | any> = object({
    ...
    title: string()
        .max(80, { title: "Gig title must be at most 80" })
        .required({ title: "Gig title is a required field" }),
    categories: string()
    ...
    coverImage: string()
        .notOneOf(["https://placehold.co/330x220?text=Cover+Image"], {
            coverImage: "Gig cover image is a required field"
        })
        .required({
            coverImage: "Gig cover image is a required field"
        }),
    expectedDelivery: string()
        .notOneOf(["Expected delivery"], {
            expectedDelivery: "Select expected delivery"
        })
        .required({
            expectedDelivery: "Gig expected delivery is a required field"
        }),
    basicDescription: string()
        .max(100, {
            basicDescription: "Gig basic description must be at most 100"
        })
        .required({
            basicDescription: "Gig basic description is a required field"
        }),
    basicTitle: string()
