...
import { useGetReviewsByGigIdQuery } from "src/features/order/service/review.service";
...
const GigViewReviews: FC<IGigViewReviewsProps> = ({
  showRatings,
  reviews,
  hasFetchedReviews
}): ReactElement => {
  ...
  return (
    <>
          ...
          <div className="mb-10">
            ...
            <div className="flex flex-col gap-y-3 pt-2 lg:flex-row lg:gap-x-6">
              <div className="w-full">
                {Object.entries(gig?.ratingCategories as IRatingCategories).map(
                  (rating: [string, IRatingCategoryItem]) => (
                    <div
                      key={uuidv4()}
                    ...
                    >
                      ...
                      <div className="flex h-2.5 w-full self-center rounded-full bg-slate-200 lg:w-full">
                        <div
                          ...
                          style={{
                            width: `${percentage(rating[1].value, parseInt(`${gig?.ratingSum}`))}%`
                          }}
                        ></div>
                      </div>
      ...
      <div className="flex flex-col gap-6">
        {reviews &&
          reviews
            .filter((item) => item.reviewerId !== seller._id)
            .map((item: IReviewDocument) => (
              <div
                key={uuidv4()}
                className="flex flex-col gap-y-3 md:flex-row md:gap-x-4"
              >
                <img
