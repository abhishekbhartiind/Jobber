...
import { IGigTopProps, ISellerGig } from "../../interfaces/gig.interface";
import {
  useGetGigByIdQuery,
  useGetMoreGigsLikeThisQuery
} from "../../service/gig.service";
import GigViewLeft from "./components/GigViewLeft";
import GigViewRight from "./components/GigViewRight";

const TopGigsView: LazyExoticComponent<FC<IGigTopProps>> = lazy(
  () => import("src/shared/gigs/TopGigsView")
);
...
const GigView: FC = (): ReactElement => {
  ...
  } = useGetGigByIdQuery(gigId!, { refetchOnMountOrArgChange: true });
  const {
    data: sellerData,
  ...
  } = useGetMoreGigsLikeThisQuery(String(gigId));
                ...
                <div className="flex w-full gap-x-1 self-center">
                  ...
                  <div className="ml-2 mt-[1px] flex gap-1 text-sm">
                    <span className="text-orange-400">
                      {rating(gig.current.ratingSum / gig.current.ratingsCount)}
                    </span>
                    <span className="">
