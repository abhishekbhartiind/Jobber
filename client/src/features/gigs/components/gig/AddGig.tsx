...
import {
  ...
  IModalProps
} from "src/shared/modal/interfaces/modal.interface";
...
import { useGigSchema } from "../../hook/useGigSchema";
import { useCreateGigMutation } from "../../service/gig.service";
import { gigInfoSchema } from "../../schema/gig.schema";
...
const AddGig: FC = (): ReactElement => {
  ...
  const [schemaValidation, validationErrors] = useGigSchema({
  ...
  const onCreateGig = async (): Promise<void> => {
    try {
      ...
      const gig: ICreateGig = {
        profilePicture: authUser.profilePicture?.toString(),
        sellerId,
        title: gigInfo.title,
      ...
      navigate(
        `/gig/${lowerCase(String(authUser.username))}/${title}/${response?.gig?.sellerId}/${response?.gig?.id}/view`
      );
    } catch (error) {
      showErrorToast("Error creating gig");
    }
  ...
  const onCancelCreate = (): void => {
    navigate(
      `/seller_profile/${lowerCase(`${authUser.username}/${sellerId}/edit`)}`
    );
  };
        ...
        <div className="container relative mx-auto my-5 px-2 pb-12 md:px-0">
          {validationErrors.length > 0 && (
            <div className="text-red-400">
              <h3 className="text-center">{`You have ${validationErrors.length} error${validationErrors.length > 1 ? "s" : ""}`}</h3>
              <ul className="list-disc ms-6">
                {validationErrors.map((error, index) => (
                  <li key={index}>
                    {typeof error === "object"
                      ? (Object.values(error)[0] as string)
                      : (error as string)}
                  </li>
                ))}
              </ul>
                ...
                <TextInput
                  ...
                  onChange={(event: ChangeEvent) => {
                    ...
                    const counter: number =
                      GIG_MAX_LENGTH.gigTitle - gigTitleValue.length;
              ...
              <div className="col-span-4 md:w-11/12 lg:w-8/12">
                <TextInput
                  ...
                  onChange={(event: ChangeEvent) => {
                    ...
                    const counter: number =
                      GIG_MAX_LENGTH.basicTitle - basicTitleValue.length;
                    setAllowedGigItemLength({
                      ...allowedGigItemLength,
              ...
              <div className="col-span-4 md:w-11/12 lg:w-8/12">
                <ReactQuill
                  ...
                  ref={(element: ReactQuill | null) => {
                    ...
                    const reactQuillEditor: Quill | undefined =
                  ...
                  onChange={(
                    ...
                    const counter: number =
                      GIG_MAX_LENGTH.fullDescription - editor.getText().length;
                    setAllowedGigItemLength({
                      ...allowedGigItemLength,
            ...
            <div className="mb-12 grid md:grid-cols-5">
              ...
              <div className="relative col-span-4 md:w-11/12 lg:w-8/12">
                ...
                <TextInput
                  type="number"
                  className="border-grey mb-1 w-full rounded border p-3.5 text-sm font-normal text-gray-600 focus:outline-none"
                  placeholder="Enter delivery estimated time (day /s)"
                  name="expectedDelivery"
                  value={`${gigInfo.expectedDelivery}`}
                  onChange={(event: ChangeEvent) => {
                    const value: string = (event.target as HTMLInputElement)
                      .value;
                    setGigInfo({
