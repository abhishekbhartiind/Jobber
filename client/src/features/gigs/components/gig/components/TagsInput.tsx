...
const TagsInput: FC<ITagsInputProps> = (props): ReactElement => {
  ...
  const deleteTag = (index: number): void => {
    setItem((prevState: string[]) =>
      prevState.filter((_, i: number) => i !== index)
    );
    const gigInfoList: string[] = gigInfo[itemName] as string[];
    setGigInfo({
      ...gigInfo,
      [itemName]: gigInfoList.filter((_, i: number) => i !== index)
    });
  };
      ...
      <div className="col-span-4 md:w-11/12 lg:w-8/12">
        ...
        <TextInput
          ...
          onKeyDown={(event: KeyboardEvent) =>
            onKeyDown(event, itemInput, tags)
          }
          onKeyUp={onKeyUp}
        />
        <span className="flex justify-between">
          <span className="text-[10px]">
            press <strong className="text-xs font-black">Enter</strong> to add
          </span>
