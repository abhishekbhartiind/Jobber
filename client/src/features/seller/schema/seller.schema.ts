...
const personalInfoSchema: ObjectSchema<IPersonalInfoData> = object({
    ...
    responseTime: string().required({
        responseTime: "Response time is a required field"
    }),
...
const experienceSchema: ObjectSchema<IExperience> = object({
    ...
    endDate: string()
        .notOneOf(["End Year"], { endDate: "Select an end year" })
        .required({ endDate: "End year is a required field" }),
    description: string().required({
...
const educationSchema: ObjectSchema<IEducation> = object({
    _id: string().optional(),
    country: string()
        .notOneOf(["Country"], { country: "Select a country" })
        .required({ country: "Country is a required field" }),
    university: string().required({
        university: "University is a required field"
    }),
    title: string()
        .notOneOf(["Title"], { title: "Add a title" })
        .required({ title: "Title is a required field" }),
    major: string().required({ major: "Major is a required field" }),
    year: string()
        .notOneOf(["Year"], { year: "Select a year" })
...
const languagesSchema: ObjectSchema<ILanguage> = object({
    ...
    level: string()
        .notOneOf(["Level"], { level: "Select a level" })
        .required({ level: "level is a required field" })
});
