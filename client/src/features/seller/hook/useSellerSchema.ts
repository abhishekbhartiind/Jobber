...
const useSellerSchema = ({
    ...
    async function schemaValidation(): Promise<boolean> {
        ...
        await ArrayOfExperienceSchema.validate(experienceFields, {
            abortEarly: false
        })
        ...
        await ArrayOfEducationSchema.validate(educationFields, {
            abortEarly: false
        })
        ...
        await ArrayOfLanguagesSchema.validate(languageFields, {
            abortEarly: false
        })
        ...
        const isPersonalInfoValid = await personalInfoSchema.isValid(
            personalInfo,
            { abortEarly: false }
        );
        const isExperienceValid = await ArrayOfExperienceSchema.isValid(
            experienceFields,
            { abortEarly: false }
        );
        const isEducationValid = await ArrayOfEducationSchema.isValid(
            educationFields,
            { abortEarly: false }
        );
        const isSkillValid = await ArrayOfSkillsSchema.isValid(skillsFields, {
            abortEarly: false
        });
        const isLanguageValid = await ArrayOfLanguagesSchema.isValid(
            languageFields,
            { abortEarly: false }
        );
