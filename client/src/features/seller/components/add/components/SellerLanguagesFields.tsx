...
const SellerLanguageFields: FC<ILanguageProps> = ({
  languageFields,
  setLanguageFields
}): ReactElement => {
  ...
  const handleLanguageFieldsChange = (
    event: ChangeEvent,
    index: number
  ): void => {
    if (languageFields && setLanguageFields) {
    ...
    <>
      <div className="border-grey flex w-full flex-col border-b px-6 pb-3 pt-6">
        ...
        {languageFields?.map((input: ILanguage, index: number) => (
          <div
            key={index}
            className="grid grid-cols-1 gap-y-3 md:grid-cols-2 md:gap-x-2"
          >
            <div className="">
              <TextInput
                ...
                onChange={(event: ChangeEvent) =>
                  handleLanguageFieldsChange(event, index)
                }
              />
            </div>
