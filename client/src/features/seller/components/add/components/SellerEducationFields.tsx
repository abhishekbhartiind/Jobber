...
const SellerEducationFields: FC<IEducationProps> = ({
  educationFields,
  setEducationFields
}): ReactElement => {
  const handleEducationFieldsChange = (
    event: ChangeEvent,
    index: number
  ): void => {
    ...
    <div className="border-grey flex w-full flex-col border-b px-6 pb-3 pt-6">
      <div className="flex justify-between">
        <h2 className="pb-4 text-xl font-bold">
          Education<sup className="top-[-0.3em] text-base text-red-500">*</sup>
        </h2>
        <Button
      ...
      {educationFields?.map((input: IEducation, index: number) => (
        <div
          key={index}
          style={{ zIndex: (educationFields.length - index) * 1000 }}
        >
          <div className="relative">
            <TextInput
              ...
              onChange={(event: ChangeEvent) =>
                handleEducationFieldsChange(event, index)
              }
            />
          </div>
          ...
          <div className="mt-4 grid h-1/5 grid-cols-4 gap-x-2 gap-y-3">
            <div className="relative">
              <Dropdown
                ...
                // style={{ zIndex: `${ + educationFields.length - index}` }}
                values={degreeList()}
                onClick={(item: string) => {
                  const data: IEducation[] = [...educationFields];
