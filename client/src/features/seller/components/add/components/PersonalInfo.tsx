...
import { IPersonalInfoProps } from "src/features/seller/interfaces/seller.interface";
import TextAreaInput from "src/shared/input/TextAreaInput";
import TextInput from "src/shared/input/TextInput";

const PersonalInfo: FC<IPersonalInfoProps> = ({
  personalInfo,
  setPersonalInfo
}): ReactElement => {
      ...
      <div className="grid md:grid-cols-5 mb-6">
        ...
        <div className="w-full col-span-4">
          <TextInput
            ...
            onChange={(event: ChangeEvent) => {
              const onelinerValue: string = (event.target as HTMLInputElement)
                .value;
              setPersonalInfo({ ...personalInfo, oneliner: onelinerValue });
              const counter: number =
                maxOneLinerCharacters - onelinerValue.length;
              setAllowedInfoLength({
                ...allowedInfoLength,
                oneliner: `${counter}/${maxOneLinerCharacters}`
              });
            }}
            maxLength={maxOneLinerCharacters}
            required={true}
          />
          <span className="flex justify-end text-[#95979d] text-xs">
            {allowedInfoLength.oneliner} Characters
          </span>
        </div>
      </div>
      <div className="grid md:grid-cols-5 mb-6">
        <div className="text-base font-medium pb-2">
          Description
          <sup className="text-red-500 text-base top-[-0.3em]">*</sup>
        </div>
        <div className="w-full col-span-4">
          <TextAreaInput
            className="w-full rounded border border-grey p-2.5 mb-1 text-sm font-normal text-gray-600 focus:outline-none"
            ...
            onChange={(event: ChangeEvent) => {
              ...
              const counter: number =
                maxDescriptionCharacters - descriptionValue.length;
              setAllowedInfoLength({
                ...allowedInfoLength,
                description: `${counter}/${maxDescriptionCharacters}`
              });
            }}
          ...
          />
          <span className="flex justify-end text-[#95979d] text-xs">
            {allowedInfoLength.description} Characters
          </span>
        </div>
      </div>
      <div className="grid md:grid-cols-5 mb-6">
        ...
        <div className="w-full col-span-4">
          <TextInput
            ...
            onChange={(event: ChangeEvent) => {
              ...
              setPersonalInfo({
                ...personalInfo,
                responseTime: parseInt(value) > 0 ? value : ""
              });
            }}
            required={true}
          />
