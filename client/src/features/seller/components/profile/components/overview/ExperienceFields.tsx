...
const ExperienceFields: FC<IExperienceEditProps> = ({
  ...
  const [startDate, setStartDate] = useState<string>(
    selectedExperience?.startDate ?? "Start Year"
  );
  const [endDate, setEndDate] = useState<string>(
    selectedExperience?.endDate ?? "End Year"
  );
  ...
  const onHandleUpdate = () => {
    if (type === "add") {
      ...
      const clonedExperience: IExperience[] = cloneDeep(
        sellerProfile?.experience
      ) as IExperience[];
      ...
      const itemIndex: number = findIndex(
        sellerProfile?.experience,
        (value: IExperience) => value._id === selectedExperience?._id
      );
      const clonedExperience: IExperience[] = cloneDeep(
        sellerProfile?.experience
      ) as IExperience[];
      ...
      const filtered: IExperience[] = clonedExperience.filter(
        (item: IExperience) => item.title !== "" && item.company !== ""
      );
          ...
          <div className="mb-4 mt-2 flex items-center">
            <TextInput
              ...
              onChange={(event: ChangeEvent) => {
                ...
                setExperienceItem({
                  ...experienceItem,
                  currentlyWorkingHere: (event.target as HTMLInputElement)
                    .checked
                });
              }}
            />
