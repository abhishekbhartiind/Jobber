...
const CertificateEditFields: FC<ICertificateEditProps> = ({
  ...
  const [certificateItem, setCertificateItem] = useState<ICertificate>({
    name:
      selectedCertificate && selectedCertificate.name
        ...
        : "",
    from:
      selectedCertificate && selectedCertificate.from
        ? selectedCertificate.from
        : "",
    year:
      selectedCertificate && selectedCertificate.year
  ...
  const onHandleUpdate = (): void => {
    ...
    if (type === "add") {
      ...
      const clonedCertificates: ICertificate[] = cloneDeep(
        sellerProfile.certificates
      ) as ICertificate[];
      ...
      const itemIndex: number = findIndex(
        sellerProfile?.certificates,
        (value: ICertificate) => value.name === selectedCertificate?.name
      );
      const clonedCertificates: ICertificate[] = cloneDeep(
        sellerProfile?.certificates
      ) as ICertificate[];
