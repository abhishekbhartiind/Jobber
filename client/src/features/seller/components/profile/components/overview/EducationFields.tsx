...
const EducationFields: FC<IEducationEditProps> = ({
  ...
  const [country, setCountry] = useState<string>(
    selectedEducation?.country ?? "Country"
  );
  const [university, setUniversity] = useState<string>(
    selectedEducation?.university ?? ""
  );
  const [title, setTitle] = useState<string>(
    selectedEducation?.title ?? "Title"
  );
  ...
  const onHandleUpdate = (): void => {
    if (type === "add") {
      ...
      const clonedEducation: IEducation[] = cloneDeep(
        sellerProfile?.education
      ) as IEducation[];
      ...
      const itemIndex: number = findIndex(
        sellerProfile?.education,
        (value: IEducation) => value._id === selectedEducation?._id
      );
      const clonedEducation: IEducation[] = cloneDeep(
        sellerProfile?.education
      ) as IEducation[];
      ...
      const filtered: IEducation[] = clonedEducation.filter(
        (item: IEducation) => item.university !== "" && item.major !== ""
      );
        ...
        <div className="relative">
          <TextInput
            ...
            onChange={(event: ChangeEvent) =>
              setUniversity((event.target as HTMLInputElement).value)
            }
          />
        </div>
        ...
        <div className="mt-4 grid h-1/5 grid-cols-4 gap-x-2 gap-y-3">
          ...
          <div className="col-span-2">
            <TextInput
              ...
              value={major}
              onChange={(event: ChangeEvent) =>
                setMajor((event.target as HTMLInputElement).value)
              }
            />
          </div>
          <div className="relative">
