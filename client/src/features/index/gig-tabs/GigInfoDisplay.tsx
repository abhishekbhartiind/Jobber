...
import { useGetAuthGigByIdQuery } from "src/features/auth/service/auth.service";
...
const LoginModal: LazyExoticComponent<FC<IModalBgProps>> = lazy(
  () => import("src/features/auth/components/Login")
);
const RegisterModal: LazyExoticComponent<FC<IModalBgProps>> = lazy(
...
const GigInfoDisplay: FC = (): ReactElement => {
  ...
  function processPurchasingGig() {
    setShowModal({ ...showModal, register: true });
    saveToLocalStorage("upgoingPurchasingGig", JSON.stringify(gig.current));
  }
