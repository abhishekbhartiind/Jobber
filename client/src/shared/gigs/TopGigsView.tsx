...
const TopGigsView: FC<IGigTopProps> = ({
  gigs,
  title,
  subTitle,
  category,
    ...
    if (navElement.current) {
      ...
      navElement.current.scrollLeft =
        navElement.current.scrollLeft < maxScrollLeft
          ...
          : maxScrollLeft;
      const maxWidth =
        navElement.current.scrollLeft + navElement.current.clientWidth;
      setScroll({
    ...
    if (navElement.current) {
      ...
      navElement.current.scrollLeft =
        navElement.current.scrollLeft < maxScrollLeft
          ...
          : maxScrollLeft;
      const maxWidth =
        navElement.current.scrollLeft + navElement.current.clientWidth - 1000;
      setScroll({
        start: true,
  ...
  return (
    <div className="mx-auto my-4 flex flex-col overflow-hidden rounded-lg">
      {title && (
      ...
      <div
        className="m-auto flex h-full w-full overflow-x-auto"
        ref={navElement}
      >
        {scroll.start && gigs.length > 2 && (
          <span
            onClick={slideLeft}
            className="absolute left-2 z-50 flex cursor-pointer justify-start self-center rounded-full bg-sky-400 sm:left-3 md:left-7"
          >
        ...
        {!scroll.end && gigs.length > 2 && (
          <span
            onClick={slideRight}
            className="absolute right-2 flex max-w-4xl cursor-pointer justify-end self-center rounded-full bg-sky-400 sm:right-3 md:right-7"
          >
