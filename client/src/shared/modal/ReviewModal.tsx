...
import { useAddReviewMutation } from "src/features/order/service/review.service";
import {
  ...
  const onAddReview = async (): Promise<void> => {
    try {
      ...
      const reviewDocument: IReviewDocument = {
        gigId: `${order?.gigId}`,
        reviewerId:
          type === "buyer-review" ? `${order?.buyerId}` : `${order?.sellerId}`,
      ...
      const response = await addReview({ body: reviewDocument });
