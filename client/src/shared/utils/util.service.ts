...
import { authApi } from "src/features/auth/service/auth.service";
...
export const applicationLogout = (
    ...
    if (loggedInUsername) {
        dispatch(
            authApi.endpoints.removeLoggedInUser.initiate(
                `${loggedInUsername}`,
                { track: false }
            ) as never
        );
...
export const formatErrorObject = (
    errorList: Record<string, string | number>[]
): Record<string, string | number> => {
    const errorsObject = errorList.reduce(
        (
            acc: Record<string, string | number>,
            error: Record<string, string | number>
        ) => {
            const key: string = Object.keys(error)[0]; // Get the key (property) of the error
            const value = error[key]; // Get the corresponding error message
...
export function convertDateTimeHourAndMinutes(date: string): string {
    const dateFromDatabase = new Date(date);

    const hours = dateFromDatabase.getHours();
    const minutes = dateFromDatabase.getMinutes();
