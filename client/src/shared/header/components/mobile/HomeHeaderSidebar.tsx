...
const HomeHeaderSidebar: FC<IHeaderSideBarProps> = ({
  ...
  const settings: ISettings[] = [
    {
      id: 1,
      name: "Add a new gig",
      url: `/manage_gigs/new/${seller?._id}`,
      show: isSeller
    },
    {
      id: 2,
      name: "Dashboard",
      url: `/users/${buyer?.username}/${buyer?._id}/orders`,
      show: true
    },
    {
      id: 3,
      name: "Profile",
      url: `/seller_profile/${lowerCase(`${seller?.username}`)}/${seller?._id}/edit`,
      show: isSeller
    },
    {
      id: 4,
      name: "Settings",
      url: `/${lowerCase(`${seller?.username}`)}/edit`,
      show: true
    }
  ];
  ...
  return (
    <div
      className={
        "fixed left-0 top-0 z-[150] flex min-h-screen w-full bg-black/40 transition-all duration-500"
      }
      onClick={() => {
        if (setOpenSidebar) {
    ...
    >
      <div
        className={
          "absolute left-0 top-0 z-20 flex min-h-screen w-[250px] flex-col items-start justify-start gap-4 bg-white p-6"
        }
      >
