...
} from "src/features/order/service/order.service";
...
const OrderDropdown: FC<IHomeHeaderProps> = ({
  ...
  if (buyer) {
    ...
    const { data, isSuccess } = useGetOrdersBySellerIdQuery(seller?._id!);
                ...
                <div className="w-full pl-3">
                  ...
                  <div className="flex gap-2 text-[11px]">
                    <span className="font-normal text-[#b5b6ba]">
                      by {order.sellerUsername}
                    </span>
                    <span className="font-normal">&#x2022;</span>
                    <span
                      className={`rounded text-white px-2 ${lowerCase(order.status.replace(/ /g, ""))}`}
                    >
                      {order.status}
                    </span>
                  </div>
