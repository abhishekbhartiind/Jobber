...
import { categories, saveToLocalStorage } from "src/shared/utils/util.service";
...
const HeaderSideBar: FC<IHeaderSideBarProps> = ({
  ...
  return (
    <div
      className={
        "fixed left-0 top-0 z-40 h-screen w-full bg-black/40 transition-all duration-500 flex"
      }
      onClick={() => {
        if (setShowRegisterModal && setShowLoginModal && setOpenSidebar) {
          setShowRegisterModal((item: IHeaderModalProps) => ({
            ...item,
            register: false
          }));
          setShowRegisterModal((item: IHeaderModalProps) => ({
        ...
        <div className="z-2 sticky top-0 flex w-full flex-col items-start justify-start gap-6 bg-white">
          ...
          <div className="cursor-pointer text-base font-medium flex flex-col w-full text-gray-400">
            <span className="flex justify-between" onClick={toggleDropdown}>
              Browse Categories{" "}
              {!isDropdownOpen ? (
                <FaAngleDown className="flex self-center mt-1" />
              ) : (
                <FaAngleUp className="flex self-center mt-1" />
              )}
            </span>
            <div className="my-2">
              <Transition
              ...
              >
                <ul>
                  {categories().map((category: string) => (
                    <li
                      key={uuidv4()}
                      className="py-2 text-right flex justify-between cursor-pointer hover:text-sky-400"
                    >
